...
class Director(Resource):
    ...
    @director_ns.doc(params={'director_id': 'An ID'})
    ...
    @director_ns.response(201, 'Created', director_model)
    ...
    @director_ns.response(200, 'Successfully update')
    @director_ns.response(400, 'Validation Error')
    @director_ns.response(404, 'Not Found')
    @director_ns.doc(params={'director_id': 'An ID'}, model=director_model, body=director_model)
    def put(self, director_id):
        """Update a record in the director table"""
        return todo.update(record_id=director_id, crud=director, t_name='director')

    @director_ns.response(204, 'Record deleted successfully')
    @director_ns.response(404, 'Not Found')
    @director_ns.doc(params={'director_id': 'An ID'})
